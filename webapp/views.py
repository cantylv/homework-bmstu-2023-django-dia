...
def contractors(req):  # тут нужно дописать логику с get-параметрами, идущими в качестве параметров
    catalog_id = int(req.GET.get('catalog_id', -1))
    items = [item for item in CONTRACTORS if item['catalog_id'] == catalog_id]
    return render(req, 'public/pages/executors_listing.html',{
        'contractors': items
    })


def contractor(req, contractor_id):  # тут нужно дописать логику с get-параметрами, идущими в качестве параметров
    selected_contractor = next((item for item in CONTRACTORS if item['id'] == contractor_id), -1)
